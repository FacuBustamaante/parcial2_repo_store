      ...
      <section className='max-w-7xl mx-auto p-6 bg-(--bg)'>
         ...
         <div className='flex items-center gap-4 mb-6'>
            {["Todos", ...categories].map((cat) => (
               <button
                  key={cat}
                  className={`px-4 py-2 border-1 border-(--line) rounded-full text-sm font-medium ${activeCat === cat ? 'bg-(--gold) text-white' : 'bg-(--bg) text-(--text) hover:bg-white hover:text-(--bg) transition-colors duration-300  '}`}
